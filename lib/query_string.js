...
async function getIntent(query) {
    try {
        ...
        console.log("Intent '%s' | Confidence %s", data.intents[0].slug, data.intents[0].confidence);
        return {
...
async function getMessageFromDatabase(query) {
    try {
        ...
        if (intent.confidence > 0.4) {
            // Si el intento se tiene que tenerle en cuenta sus entitades, hacer lo siguiente... 
            if (intent.slug == "grado-plan-estudios") {
                // Guardar entidad con nombre del grado del objecto 'entities' devuelto por Recast
                const grado = query.faculty.toLowerCase() + "-grado-";
                var nombreEntitdadGrado;
                var posicionObjectoEntidadGrado = -1;
                ...
                for (var i = 0; i < intent.entities.length; i++) {
                    if ((intent.entities[i]).includes(grado)) {
                        posicionObjectoEntidadGrado = i;
                        nombreEntitdadGrado = intent.entities[i];
                    }
                };

                console.log("Nombre entidad del grado: %s", nombreEntitdadGrado);
