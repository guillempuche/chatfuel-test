...
async function getFormattedMessage(query) {
    ...
    try {
        const chatfuelMessage = await require('./chatfuel').getChatfuelFormat(data);
...
module.exports.getResponse = async function getResponse(request) {
    ...
    try {
        ...
        console.log("Mensaje para Chatfuel:", JSON.stringify(message));
        return message;
    } catch(err) {
